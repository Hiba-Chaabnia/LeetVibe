    ...
    def _run_loop(self, tools: list[dict] | None = None) -> Generator[str, None, None]:
        ...
        for _ in range(max_turns):
            ...
            for tc in tool_calls_acc:
                ...
                yield f"\n[dim]│ Tool call:[/dim] [bold dim]{icon} {name}[/bold dim]\n"
                try:
                ...
                # Show a compact preview of the result (collapse newlines, strip
                # markdown markers so they don't render literally inside [dim]...[/dim])
